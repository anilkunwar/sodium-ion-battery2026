...
# ============================================================================
st.set_page_config(
    page_title="Sodium-Ion Battery Quantitative Descriptor Graph v6.1",
    page_icon="🔋",
    layout="wide",
...
def generate_analysis_report(
    ...
    report.append("")
    report.append("---")
    report.append("*Report generated by Sodium-Ion Battery Quantitative Descriptor Graph v6.1*")
    return "\n".join(report)
    ...
    def analyze_query(self, query: str, ontology: Any) -> QueryAnalysisResult:
        ...
        try:
            ...
            if json_match:
                ...
                # ── Actually use the parsed JSON instead of discarding it ──
                self._pending_new_concepts = parsed.get("new_concepts", [])
                self._pending_new_relationships = parsed.get("new_relationships", [])
                ...
                for c in all_relevant:
                    is_explicit = c in explicitly_mentioned
                    priorities[c] = ConceptPriority(
                        concept_name=c,
                        concept_type=ontology.get_concept_type(c).value,
                ...
                return QueryAnalysisResult(
                    ...
                    suggested_layout="bisected" if parsed.get("query_type") == "comparison" else "force",
                    highlight_paths=[[p[0], p[1]] for p in parsed.get("highlight_paths", []) if isinstance(p, (list, tuple)) and len(p) >= 2],
                    visualization_focus=SIB_PROBLEM_DEFINITIONS[primary].visualization_focus,
                    reasoning_chain=parsed.get("reasoning_chain", ["Local LLM analysis completed"]),
                    confidence=0.75,
                )
        except Exception as e:
...
class GraphRAGAnswerGenerator:
    """Bullet-proof answer generator with graceful LLM-fallback chains."""

    def __init__(self, analyzer: LLMQueryAnalyzer):
        self.analyzer = analyzer

    def generate_ground_response(
        self,
        query: str,
        analysis: Optional[QueryAnalysisResult],
        subgraph: Optional[nx.Graph],
        concept_abstract_map: Dict[str, List[int]],
        all_texts: Union[List[str], Dict[int, str], None],
        max_docs_per_concept: int = 2,
    ) -> str:
        """Build evidence context and route to LLM or fallback."""
        nl = chr(10)

        # ── 1. Defensive normalisation ──────────────────────────────
        if analysis is None:
            return (
                "⚠️ **Query analysis failed.**" + nl + nl
                + "The system could not parse your question into a structured "
                + "analysis. Please try rephrasing with SIB-specific terms "
                + "(e.g., 'hard carbon', 'layered oxide cathode', 'SEI')."
            )
        ...
        safe_texts = all_texts if all_texts is not None else {}

        # ── 2. Extract top-priority nodes ────────────────────────────
        try:
            top_nodes = sorted(
                subgraph.nodes(data=True),
                key=lambda x: float(x[1].get("priority_score", 0.0) or 0.0),
                reverse=True,
            )[:5]
        except Exception:
            top_nodes = []

        # ── 3. Gather evidence snippets ──────────────────────────────
        ...
        for node, attrs in top_nodes:
            ...
            for idx in doc_indices:
                try:
                    if isinstance(safe_texts, dict):
                        text = safe_texts.get(idx, "")
                    else:
                        text = safe_texts[idx] if 0 <= idx < len(safe_texts) else ""
                except Exception:
                    text = ""
                if text:
                    try:
                        clean_text = re.sub(r"\s+", " ", str(text)).strip()[:400]
                        evidence_snippets.append("- **" + str(node) + "**: " + clean_text + "...")
                    except Exception:
                        pass

        # ── 4. Build prompt ──────────────────────────────────────────
        primary_problem = getattr(analysis, "primary_problem", None)
        problem_label = (
            primary_problem.value.replace("_", " ").title()
            if primary_problem is not None
            else "General SIB Inquiry"
        )

        prompt = (
            "You are an expert Sodium-Ion Battery (SIB) researcher. "
            "Answer the user's query based *strictly* on the provided graph context and evidence snippets."
            + nl
        )
        prompt += "User Query: " + repr(query) + nl
        prompt += "Identified Core Problem: " + problem_label + nl
        prompt += (
            "Key Graph Concepts: "
            + ", ".join([str(n) for n, _ in top_nodes])
            + nl
        )
        ...
        if evidence_snippets:
            ...
            prompt += (
                "No direct text snippets found. Rely on your general SIB knowledge "
                "but note the lack of specific retrieved context."
                + nl
            )
        ...
        # ── 5. Route to LLM or fallback ──────────────────────────────
        if isinstance(self.analyzer, OpenAIQueryAnalyzer) and self.analyzer.is_available():
            return self._call_llm_for_answer(
                prompt, self.analyzer, query, analysis, top_nodes, evidence_snippets
            )
    ...
    def _call_llm_for_answer(
        ...
        """Call OpenAI with exhaustive null-guards."""
        nl = chr(10)
        ...
        try:
            response = client.chat.completions.create(
                model=getattr(analyzer, "model", "gpt-4o-mini"),
            ...
            fallback = self._generate_fallback_answer(
                query, analysis, top_nodes, evidence_snippets
            )
            return "⚠️ LLM API Error: " + str(e) + nl + nl + fallback

        # ── Bullet-proof response unpacking ──────────────────────────
        if response is None:
            return self._generate_fallback_answer(query, analysis, top_nodes, evidence_snippets)
    ...
    def _generate_fallback_answer(
        ...
        """Structured fallback that survives *any* input state."""
        nl = chr(10)
        response = "### Analysis of: '" + str(query) + "'" + nl + nl
        ...
        if analysis is not None:
            ...
            response += (
                ...
                + (primary.value.replace("_", " ").title() if primary else "General")
                + nl
                + nl
            )
        else:
            response += "**Core Problem Identified:** (analysis unavailable)" + nl + nl
        ...
        if top_nodes:
            ...
            for item in top_nodes:
                if isinstance(item, tuple) and len(item) == 2:
                    ...
                    node = item.get("name", item.get("id", str(item)))
                    attrs = item
                else:
                    continue
                if not isinstance(attrs, dict):
                    attrs = {}
                ctype = attrs.get("concept_type", "general") or "general"
                pscore = attrs.get("priority_score", 0.0)
                if pscore is None:
                    pscore = 0.0
                lines.append(
                    "- **" + str(node) + "** (" + str(ctype) + "): Priority Score "
                    + str(round(float(pscore), 2))
                )
            response += nl.join(lines) if lines else "(none extracted)"
        else:
            response += "(none extracted)"
        response += nl

        # ── Evidence line ────────────────────────────────────────────
        safe_snippets = snippets if isinstance(snippets, (list, tuple)) else []
        if safe_snippets:
            response += nl + "**Retrieved Evidence Context:**" + nl
            for i, s in enumerate(safe_snippets[:3], 1):
                response += str(i) + ". " + str(s)[:500] + "..." + nl
        else:
            response += (
                nl
                + "*Note: No direct text snippets were linked to these "
                "concepts in the current dataset.*"
                + nl
            )

        # ── Reasoning chain ──────────────────────────────────────────
        if analysis is not None:
            reasoning = getattr(analysis, "reasoning_chain", None)
            if reasoning:
                response += nl + "**System Reasoning Chain:**" + nl
                response += nl.join(["- " + str(step) for step in reasoning])
...
class QuerySessionManager:
    ...
    @classmethod
    def record_query(cls, query: str, analysis: Optional[QueryAnalysisResult], mutations: Dict[str, Any]) -> None:
        session = cls.init_session()
        session["query_history"].append(query)
        if analysis is not None:
            session["analysis_history"].append({
                "query": query,
                "primary_problem": getattr(getattr(analysis, "primary_problem", None), "value", "general"),
                "query_type": getattr(analysis, "query_type", "general"),
                "concepts_found": len(getattr(analysis, "all_relevant_concepts", [])),
                "explicit": len(getattr(analysis, "explicitly_mentioned", [])),
                "inferred": len(getattr(analysis, "inferred_concepts", [])),
                "confidence": getattr(analysis, "confidence", 0.0),
                "timestamp": datetime.now().isoformat(),
            })
        else:
            session["analysis_history"].append({
                "query": query, "primary_problem": "unknown", "query_type": "unknown",
                "concepts_found": 0, "explicit": 0, "inferred": 0, "confidence": 0.0,
                "timestamp": datetime.now().isoformat(),
            })
...
def render_llm_query_panel(ontology: Any, expander: DynamicOntologyExpander, full_graph: nx.Graph) -> Optional[QueryAnalysisResult]:
    ...
    try:
        ...
        if analysis is None:
            st.sidebar.error("Analyzer returned no result.")
            return None
    ...
    if mutations.get("bridges_created"):
        st.sidebar.info(f"🌉 {len(mutations['bridges_created'])} bridge concept(s) created")
        for b in mutations["bridges_created"]: st.sidebar.markdown(f"  - `{b.get('bridge', '?')}` ← `{b.get('for', '?')}`")
    return analysis
...
def render_llm_qa_tab(analysis_data: Dict, ontology: Any):
    ...
    st.markdown(
        "Ask a specific scientific question. The system will dynamically expand the ontology, "
        "extract a relevant subgraph, and generate a grounded answer using retrieved literature snippets."
    )
    ...
    if "qa_generator" not in st.session_state:
        st.session_state.qa_generator = GraphRAGAnswerGenerator(
            st.session_state.qa_factory.get_analyzer("auto")
        )
    ...
    with col1:
        query = st.text_input(
            "Enter your SIB research question:",
            placeholder="e.g., Why does hard carbon suffer from low initial Coulombic efficiency?",
        )
    with col2:
    ...
    if st.button("🔍 Analyze & Answer", type="primary"):
        ...
        analysis: Optional[QueryAnalysisResult] = None
        mutations: Dict[str, Any] = {"concepts_added": [], "relationships_added": [], "bridges_created": []}
        try:
            ...
            st.info("Falling back to a generic answer based on the full graph.")
            # Build a minimal dummy analysis so the rest of the pipeline doesn't crash
            analysis = FallbackAnalyzer().analyze_query(query, ontology)

        # ── Stage 2: Subgraph extraction ─────────────────────────────
        subgraph: Optional[nx.Graph] = None
        try:
            with st.spinner("🕸️ Extracting priority-guided subgraph..."):
                full_graph = analysis_data.get("nx_graph")
        ...
        try:
            with st.spinner("📚 Retrieving evidence and generating answer..."):
                answer = generator.generate_ground_response(
                    ...
                    analysis=analysis,
                    subgraph=subgraph,
                    concept_abstract_map=analysis_data.get("concept_abstract_map", {}),
            ...
            answer = (
                ...
                "The graph data may be incomplete for this query."
            )
        ...
        try:
            ...
            html = visualizer.render_pyvis(
                subgraph if subgraph is not None else nx.Graph(),
                analysis if analysis is not None else QueryAnalysisResult(
                    original_query=query, normalized_query=query.lower().strip(),
                    primary_problem=SIBCoreProblem.GENERAL, secondary_problems=[],
                    problem_confidences={}, explicitly_mentioned=[], inferred_concepts=[],
                    all_relevant_concepts=[], concept_priorities={}, query_type="general",
                    emphasis_direction="neutral", comparison_pairs=[], subgraph_depth=2,
                    priority_threshold=0.3, focus_nodes=[], bridge_nodes=[],
                    suggested_layout="force", highlight_paths=[], visualization_focus=[],
        ...
        with st.expander("🔧 Behind the Scenes: Ontology Mutations & Reasoning"):
            if analysis is not None:
                ...
                for step in getattr(analysis, "reasoning_chain", []) or []:
            ...
            if mutations.get("concepts_added") or mutations.get("bridges_created"):
                ...
                for c in mutations.get("concepts_added", []):
                    st.markdown("➕ Added Concept: `" + str(c.get("name", "?")) + "` (" + str(c.get("type", "?")) + ")")
                for b in mutations.get("bridges_created", []):
                    st.markdown("🌉 Created Bridge: `" + str(b.get("bridge", "?")) + "` ← `" + str(b.get("for", "?")) + "`")
def main() -> None:
    st.title(
