...
def extract(self, analysis: QueryAnalysisResult) -> nx.Graph:
        ...
        # Only keep seeds that actually exist in the graph
        ...
        while frontier:
            ...
            # Score every unvisited neighbor
            neighbor_scores = []
            for neighbor in self.full_graph.neighbors(current):
                if neighbor in visited:
                    continue
                if neighbor in boosted:
                    score = boosted[neighbor].composite_score
                elif (neighbor in self.expander.session_concepts_added or
                      neighbor in self.expander.query_bridge_concepts):
                    score = 0.55
                else:
                    score = 0.0

                # Relaxed gate: allow medium-relevance neighbors to be considered
                if score >= threshold * 0.25 or current_depth < max_depth - 1:
                    neighbor_scores.append((neighbor, score))

            # Cap expansion per depth to keep the subgraph tight
            neighbor_scores.sort(key=lambda x: x[1], reverse=True)
            max_neighbors = {0: 6, 1: 4, 2: 2}.get(current_depth, 2)

            for neighbor, score in neighbor_scores[:max_neighbors]:
        ...
        # Annotate nodes for the visualizer
        for node in subgraph.nodes():
            if node in boosted:
                bp = boosted[node]
                subgraph.nodes[node].update({
                    "priority_score": bp.composite_score,
                    "is_explicit": bp.is_explicitly_mentioned,
                    "is_inferred": bp.is_inferred,
...
def render_pyvis(self, subgraph: nx.Graph, analysis: QueryAnalysisResult, height: str = "700px",
        ...
        seed_set = set(analysis.focus_nodes + analysis.get_concepts_above_threshold())
        ...
        for node, attrs in subgraph.nodes(data=True):
            ...
            # Tiered sizing: explicit largest, low-priority smallest
            size = 15 + priority * 35
            if is_explicit:
                size *= 1.3
            elif priority < 0.4:
            ...
            # Tiered borders: explicit > seed > high-priority > peripheral
            if is_explicit:
                border_width, border_color, shape = 5, "#FF0000", "dot"      # Red
            elif is_seed:
                border_width, border_color, shape = 3, "#FFA500", "dot"      # Orange
            elif priority >= 0.7:
                border_width, border_color, shape = 2, "#4ECDC4", "dot"      # Teal
            else:
                border_width, border_color, shape = 1, "#CCCCCC", "dot"      # Grey

            title = f"<b>{node}</b><br>Type: {concept_type}<br>Priority: {priority:.2f}"
            if is_explicit:
                title += "<br>🔴 Explicitly mentioned"
            elif is_seed:
                title += "<br>🟠 Query-relevant seed"
            elif is_llm_added:
                title += "<br>🟢 LLM-inferred concept"
            ...
            net.add_node(
                ...
                shape=shape,
                title=title,
                font={"size": max(8, min(14, int(10 + priority * 6)))}
            )

        # Edge visual encoding
        for u, v, attrs in subgraph.edges(data=True):
            edge_type = attrs.get("edge_type", "unknown")
            width = attrs.get("width", 1.0) or 1.0

            # Path highlighting from analysis
            highlighted = any(
                len(p) >= 2 and ((p[0] == u and p[1] == v) or (p[1] == u and p[0] == v))
                for p in analysis.highlight_paths
            )

            # Also highlight edges between high-priority nodes
            u_priority = subgraph.nodes[u].get("priority_score", 0.2)
            v_priority = subgraph.nodes[v].get("priority_score", 0.2)
            high_priority_edge = u_priority >= 0.6 and v_priority >= 0.6

            if highlighted:
                color, width = "#FF0000", max(width, 5.0)
            elif high_priority_edge:
                color, width = "#FF8C00", max(width, 3.0)
            else:
                color, width = "#CCCCCC", 0.5
            ...
            net.add_edge(
                ...
                width=width,
                dashes=is_inferred,
                title=f"{u} → {v}<br>Type: {edge_type}<br>Inferred: {is_inferred}",
                arrows="to"
            )
...
def render_llm_qa_tab(analysis_data: Dict, ontology: Any):
    ...
    if st.button("🔍 Analyze & Answer", type="primary"):
        ...
        # Debug stats to verify the subgraph is actually focused
        full_n = full_graph.number_of_nodes()
        full_e = full_graph.number_of_edges()
        sub_n = subgraph.number_of_nodes()
        sub_e = subgraph.number_of_edges()
        seed_n = len(set(analysis.focus_nodes + analysis.get_concepts_above_threshold()))
        st.info(
            f"**Subgraph focus:** {sub_n} nodes ({sub_n/max(full_n,1)*100:.1f}% of full graph), "
            f"{sub_e} edges ({sub_e/max(full_e,1)*100:.1f}%) | Seeds: {seed_n}"
        )
        ...
        # Focus Mode: filter out peripheral low-priority nodes
        focus_mode = st.checkbox(
            "🔎 Focus Mode: Show only high-priority nodes (≥0.5)",
            value=False,
            help="Removes grey peripheral nodes so the query-relevant core stands out."
        )
        if focus_mode:
            hp_nodes = [
                n for n, a in subgraph.nodes(data=True)
                if a.get("priority_score", 0) >= 0.5
            ]
            if len(hp_nodes) >= 3:
                subgraph = subgraph.subgraph(hp_nodes).copy()
                st.caption(f"Focus mode: showing {len(hp_nodes)} high-priority nodes.")
            else:
                st.warning("Too few high-priority nodes for Focus Mode; showing full subgraph.")
