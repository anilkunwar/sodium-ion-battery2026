...
class EnhancedConceptExtractor:
    ...
    def extract_from_text(self, text: str, doc_id: int = 0, allowed_concepts: Optional[Set[str]] = None) -> List[str]:
        ...
        context_concepts = self._extract_from_context_windows(text, allowed_concepts=allowed_concepts)
    ...
    def _extract_from_context_windows(
        self, text: str, window_size: int = 100, allowed_concepts: Optional[Set[str]] = None
    ) -> Set[str]:
...
def run_batch_analysis(
    ...
    # [QD-6] Build whitelist for query-focused batch processing
    query_focused = st.session_state.get('query_focused_build', False)
    whitelist = None
    if query_focused:
        analysis = st.session_state.get('last_query_analysis')
        if analysis is not None:
            whitelist = set(analysis.explicitly_mentioned)
            whitelist.update(analysis.inferred_concepts)
            expander = st.session_state.get('qa_expander')
            if expander:
                whitelist.update(expander.session_concepts_added)
                whitelist.update(expander.query_bridge_concepts.keys())
            st.info(f"🔍 Batch mode: focusing on {len(whitelist)} concepts")
            if whitelist and len(whitelist) < 10:
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        # [QD-1] Query-Focused Graph toggle
        st.markdown("---")
        st.subheader("🔍 Query‑Focused Graph")
        st.checkbox(
            "Build graph only for current query concepts",
            key="query_focused_build",
            help="When enabled, only concepts explicitly mentioned or inferred by the last query will be extracted from the corpus. The resulting graph will contain zero noise, but it will be rebuilt from scratch each time."
        )
    ...
    if batch_mode_on and (build_clicked or batch_trigger):
        ...
        try:
            with status:
                ...
                # [QD-5] Build whitelist for query-focused graph building
                query_focused = st.session_state.get('query_focused_build', False)
                whitelist = None
                if query_focused:
                    analysis = st.session_state.get('last_query_analysis')
                    if analysis is not None:
                        ...
                        if expander:
                            whitelist.update(expander.session_concepts_added)
                            whitelist.update(expander.query_bridge_concepts.keys())
                        st.info(f"🔍 Building graph focused on {len(whitelist)} concepts: {', '.join(sorted(whitelist))}")
                        if whitelist and len(whitelist) < 10:
                            config["MIN_CONCEPT_FREQ"] = 1
                ...
                def _process_single_row(idx, row, allowed_concepts=None):
                    ...
                    if extractor is not None:
                        ...
                        concepts = extract_concepts_from_text(text)
                        # [QD-5] Filter fallback extraction by whitelist
                        if allowed_concepts is not None:
                            concepts = [c for c in concepts if c in allowed_concepts]
                    metrics: Dict[str, Any] = {}
