...
# -*- coding: utf-8 -*-
"""
SIB LLM-DIRECTED CONCEPT GRAPH - COMPLETE STANDALONE WITH JSON DATABASE INTEGRATION
==================================================================================
A complete, runnable script that wraps the SIB Concept Graph with LLM-directed
query analysis for the six core Sodium-Ion Battery problems, and now integrates
your JSON document database for literature‑driven graph building and GNN inference.

USAGE:
    python sib_llm_directed_graph.py

REQUIREMENTS:
    pip install streamlit networkx numpy matplotlib pyvis plotly torch scikit-learn pandas
...
# ============================================================================
st.set_page_config(
    page_title="SIB LLM-Directed Concept Graph with Literature GNN",
    page_icon="🔋",
    layout="wide",
...
# CONSTANTS
# ============================================================================
JSON_METADATA_DIR = "./json_metadatabase"   # Change this to your JSON folder path
...
# ============================================================================
# EDGE COLOR REGISTRY (from original)
# ============================================================================
...
# ============================================================================
# COMPLETE PROBLEM DEFINITIONS (ALL 6 PROBLEMS) – from original
# ============================================================================
...
# ============================================================================
# COMPLETE DOMAIN ONTOLOGY (from original)
# ============================================================================
...
# ============================================================================
# SPARSE GRAPHSAGE GNN (from original)
# ============================================================================
...
# ============================================================================
# LLM QUERY ANALYZER (from original, with minor adjustments)
# ============================================================================
...
# ============================================================================
# DIRECTED CONCEPT GRAPH BUILDER (from original)
# ============================================================================
...
# ============================================================================
# QUERY-ALIGNED VISUALIZER (from original)
# ============================================================================
...
# ============================================================================
# NEW: JSON DATABASE LOADING AND LITERATURE GRAPH INTEGRATION
# ============================================================================

def load_all_json_files(directory: str) -> List[Dict]:
    """Recursively load all JSON files from directory."""
    records = []
...
class LiteratureConceptExtractor:
    """TF-IDF based concept extraction mapped to DomainOntology."""

    def __init__(self, ontology: DomainOntology, max_features: int = 2000):
        self.ontology = ontology
        ...
        self.doc_concepts = {}  # doc_idx -> {concept: weight}

    def fit_transform(self, texts: Dict[int, str]) -> None:
        if not texts:
            return
        doc_ids = sorted(texts.keys())
        doc_list = [texts[k] for k in doc_ids]

        tfidf_matrix = self.tfidf_vectorizer.fit_transform(doc_list)
        feature_names = self.tfidf_vectorizer.get_feature_names_out()

        # Build fast synonym lookup
        vocab = {}
        ...
        for feat in feature_names:
            feat_clean = re.sub(r'[^a-z0-9]', '', feat.lower())
            if feat_clean in vocab:
                feat_to_concept[feat] = vocab[feat_clean]
            else:
                # partial match
                for term, canonical in vocab.items():
                    if term in feat_clean or feat_clean in term:
                        feat_to_concept[feat] = canonical
                        break
        ...
        for i, doc_id in enumerate(doc_ids):
            ...
            for j, val in enumerate(row):
                if val > 0.1 and feature_names[j] in feat_to_concept:
                    concepts[feat_to_concept[feature_names[j]]] += val
            # Normalize by max
            if concepts:
                max_val = max(concepts.values())
                self.doc_concepts[doc_id] = {
                    k: v/max_val for k, v in concepts.items() if v/max_val > 0.2
                }
...
class DocumentGraphBuilder:
    """Build graph from ontology + co-occurrences in documents."""

    def __init__(self, ontology: DomainOntology):
        self.ontology = ontology
        self.G = nx.DiGraph()

    def build_graph(self, doc_concepts: Dict[int, Dict[str, float]],
                    co_occurrence_threshold: int = 3) -> nx.Graph:
        # Add ontology nodes and edges
        for name, node in self.ontology.concepts.items():
            self.G.add_node(name, concept_type=node.concept_type.value,
                            definition=node.definition, source="ontology")
        for rel in self.ontology.relationships:
            self.G.add_edge(rel.source, rel.target, rel_type=rel.rel_type.value,
                            weight=rel.confidence, source="ontology")

        # Count co-occurrences
        co_occ = Counter()
        for concepts in doc_concepts.values():
            clist = list(concepts.keys())
            for i in range(len(clist)):
                for j in range(i+1, len(clist)):
                    pair = tuple(sorted([clist[i], clist[j]]))
                    co_occ[pair] += 1

        # Add co-occurrence edges (undirected but stored as bidirectional)
        for (c1, c2), count in co_occ.items():
            if count >= co_occurrence_threshold and c1 in self.G.nodes and c2 in self.G.nodes:
                # Only if no existing edge (prefer ontology)
                if not self.G.has_edge(c1, c2) and not self.G.has_edge(c2, c1):
                    weight = min(count / 10.0, 1.0)
                    self.G.add_edge(c1, c2, rel_type="co_occurs", weight=weight,
                                    source="literature", co_occurrence_count=count)
                    # also reverse for undirected representation in GNN
                    self.G.add_edge(c2, c1, rel_type="co_occurs", weight=weight,
                                    source="literature", co_occurrence_count=count)
        return self.G


class LiteratureGNNWrapper:
    """Train SparseGraphSAGE on the literature graph and use for query subgraph extraction."""

    def __init__(self, graph: nx.Graph, ontology: DomainOntology, hidden_dim=64):
        self.graph = graph
        self.ontology = ontology
        self.hidden_dim = hidden_dim
        self.model = None
        self.node_embeddings = None
        self.node_to_idx = {n: i for i, n in enumerate(graph.nodes())}
        self.idx_to_node = {i: n for n, i in self.node_to_idx.items()}

    def _generate_node_features(self) -> torch.Tensor:
        """Generate feature vector: type one-hot + problem relevance + degree."""
        num_nodes = len(self.graph.nodes)
        type_to_idx = {t.value: i for i, t in enumerate(ConceptType)}
        problems = list(SIB_PROBLEM_DEFINITIONS.keys())[:6]
        in_dim = len(type_to_idx) + len(problems) + 1
        ...
        for node_name in self.graph.nodes():
            ...
            # Concept type
            ctype = node_data.get('concept_type', 'general')
            if ctype in type_to_idx:
                features[idx, type_to_idx[ctype]] = 1.0

            # Problem relevance
            for p_idx, problem in enumerate(problems):
                if problem in SIB_PROBLEM_DEFINITIONS:
                    if node_name in SIB_PROBLEM_DEFINITIONS[problem].get_ontology_concepts():
                        features[idx, len(type_to_idx) + p_idx] = 1.0

            # Normalized degree
            deg = self.graph.in_degree(node_name) + self.graph.out_degree(node_name)
            features[idx, -1] = deg / max(len(self.graph.nodes), 1)

        return features

    def prepare_edges(self) -> torch.Tensor:
        """Edge list for sparse GNN (adds reverse edges)."""
        row, col = [], []
        for u, v in self.graph.edges():
            row.append(self.node_to_idx[u])
            col.append(self.node_to_idx[v])
            # add reverse for undirected information flow
            row.append(self.node_to_idx[v])
            col.append(self.node_to_idx[u])
        return torch.tensor([row, col], dtype=torch.long)

    def train_gnn(self, epochs: int = 100, lr: float = 0.01) -> Dict[str, List[float]]:
        """Unsupervised training with edge reconstruction loss."""
        x = self._generate_node_features()
        edge_index = self.prepare_edges()
        num_nodes = x.size(0)

        self.model = SparseGraphSAGE(
            num_nodes=num_nodes,
            in_dim=x.size(1),
        ...
        for epoch in range(epochs):
            ...
            # Positive edges: both directions from prepare_edges
            pos_u, pos_v = edge_index[0], edge_index[1]
            pos_score = torch.sum(embeddings[pos_u] * embeddings[pos_v], dim=1)

            # Negative sampling
            neg_v = torch.randint(0, num_nodes, (pos_v.size(0),))
            neg_score = torch.sum(embeddings[pos_u] * embeddings[neg_v], dim=1)
        ...
        self.model.eval()
        with torch.no_grad():
            self.node_embeddings = self.model(x, edge_index).numpy()
        return history

    def get_query_focused_subgraph(self, analysis: QueryAnalysisResult, top_k: int = 15) -> nx.Graph:
        """Use GNN embeddings to find nodes closest to LLM-derived seed concepts."""
        if self.node_embeddings is None:
            return self.graph

        seed_concepts = set(analysis.all_relevant_concepts)
        seed_indices = [self.node_to_idx[c] for c in seed_concepts if c in self.node_to_idx]
        if not seed_indices:
            return self.graph

        centroid = np.mean(self.node_embeddings[seed_indices], axis=0)
        similarities = cosine_similarity(self.node_embeddings, centroid.reshape(1, -1)).flatten()
        top_indices = np.argsort(similarities)[-top_k:]

        focused_nodes = [self.idx_to_node[i] for i in top_indices]
        subG = self.graph.subgraph(focused_nodes).copy()
        for node in subG.nodes():
            idx = self.node_to_idx[node]
            subG.nodes[node]["gnn_relevance"] = float(similarities[idx])
        return subG


class SIBMasterPipeline:
    """Orchestrates JSON loading, concept extraction, graph building, GNN training, and LLM queries."""

    def __init__(self, ontology: DomainOntology, json_dir: str = JSON_METADATA_DIR):
        self.ontology = ontology
        ...
        self.literature_graph = None
        self.gnn_wrapper = None
        self.analyzer = None
        self.visualizer = None

    def load_database(self) -> pd.DataFrame:
        records = load_all_json_files(self.json_dir)
        self.df = build_master_dataframe(records)
        # Prepare texts from title + abstract
        for idx, row in self.df.iterrows():
            title = str(row.get("title", ""))[:500]
            abstract = str(row.get("abstract", ""))[:1500]
            if title.strip() or abstract.strip():
                self.doc_texts[idx] = f"{title} {abstract}".strip()
        return self.df

    def extract_concepts(self) -> Dict[int, Dict[str, float]]:
        if not self.doc_texts:
            raise ValueError("No documents loaded. Call load_database() first.")
        self.extractor = LiteratureConceptExtractor(self.ontology)
        self.extractor.fit_transform(self.doc_texts)
        return self.extractor.doc_concepts

    def build_literature_graph(self, co_occurrence_threshold: int = 3) -> nx.Graph:
        if not self.extractor:
            raise ValueError("No concepts extracted. Call extract_concepts() first.")
        self.graph_builder = DocumentGraphBuilder(self.ontology)
        self.literature_graph = self.graph_builder.build_graph(
            self.extractor.doc_concepts, co_occurrence_threshold
        )
        return self.literature_graph

    def train_gnn(self, epochs: int = 100) -> Dict[str, List[float]]:
        ...
        self.gnn_wrapper = LiteratureGNNWrapper(self.literature_graph, self.ontology)
        return self.gnn_wrapper.train_gnn(epochs=epochs)

    def init_llm_analyzer(self, mode: str = "rule_based", **kwargs):
        self.analyzer = SIBQueryAnalyzer(self.ontology, mode=mode, **kwargs)
        self.visualizer = QueryAlignedVisualizer(self.ontology)

    def run_query(self, query: str, top_k_nodes: int = 15) -> Tuple[QueryAnalysisResult, nx.Graph]:
        if not self.analyzer:
            self.init_llm_analyzer()
        analysis = self.analyzer.analyze_query(query)
        if self.gnn_wrapper and self.gnn_wrapper.node_embeddings is not None:
            focused_G = self.gnn_wrapper.get_query_focused_subgraph(analysis, top_k=top_k_nodes)
        else:
            # Fallback to static ontology builder
            builder = DirectedConceptGraphBuilder(self.ontology)
            focused_G = builder.build_focused_subgraph(analysis)
        return analysis, focused_G


# ============================================================================
# DISPLAY FUNCTIONS (from original, used by UI)
# ============================================================================
...
# ============================================================================
# MAIN STREAMLIT UI (Integrated)
# ============================================================================
def main():
    """Integrated Streamlit UI with JSON database pipeline."""
    st.set_page_config(page_title="SIB LLM + Literature Graph", page_icon="🔋", layout="wide")
    ...
    def get_pipeline():
        ...
        return SIBMasterPipeline(ontology, json_dir=JSON_METADATA_DIR)
    ...
    st.markdown("""
    <style>
    .main-header { background: linear-gradient(135deg, #1a1a2e, #16213e, #0f3460); padding: 20px; border-radius: 10px; margin-bottom: 20px; }
    .problem-card { background: #2a2a4e; padding: 15px; border-radius: 8px; border-left: 4px solid #FF6B6B; margin: 10px 0; }
    </style>
    """, unsafe_allow_html=True)

    st.markdown("""
    <div class="main-header">
    <h1 style="color: white;">🔋 SIB LLM‑Directed Concept Graph with Literature GNN</h1>
    <p style="color: #B0B0B0;">Load JSON database → Build co‑occurrence graph → Train GNN → Query</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if st.button("Load JSON Database", type="primary"):
            with st.spinner("Loading..."):
                pipeline.load_database()
            st.success(f"Loaded {len(pipeline.df)} documents.")

        st.markdown("---")
        st.header("🧠 Graph & GNN")
        co_oc_thresh = st.slider("Co‑occurrence threshold", 1, 10, 3)
        if st.button("Extract Concepts & Build Graph"):
            with st.spinner("Extracting..."):
                pipeline.extract_concepts()
                pipeline.build_literature_graph(co_occurrence_threshold=co_oc_thresh)
            st.success(f"Graph: {pipeline.literature_graph.number_of_nodes()} nodes, "
                       f"{pipeline.literature_graph.number_of_edges()} edges.")

        gnn_epochs = st.slider("GNN epochs", 50, 500, 100, step=50)
        if st.button("Train SparseGraphSAGE"):
            with st.spinner(f"Training {gnn_epochs} epochs..."):
                history = pipeline.train_gnn(epochs=gnn_epochs)
            st.success("Training complete.")
            st.line_chart(history["loss"])

        st.markdown("---")
        st.header("💬 LLM Mode")
        llm_mode = st.selectbox("Mode", ["rule_based", "local", "api"])
        # Additional LLM settings if needed
        if llm_mode == "api":
            api_key = st.text_input("OpenAI API Key", type="password")
            api_model = st.text_input("Model", value="gpt-3.5-turbo")
            pipeline.init_llm_analyzer(mode=llm_mode, api_key=api_key, api_model=api_model)
        elif llm_mode == "local":
            model_name = st.text_input("Model name", value="gpt2")
            pipeline.init_llm_analyzer(mode=llm_mode, model_name=model_name)
        else:
            pipeline.init_llm_analyzer(mode=llm_mode)

        st.markdown("---")
        st.header("📊 Status")
        st.metric("Documents", len(pipeline.df))
        st.metric("Graph Nodes", pipeline.literature_graph.number_of_nodes() if pipeline.literature_graph else 0)
        st.metric("GNN Trained", "Yes" if pipeline.gnn_wrapper and pipeline.gnn_wrapper.node_embeddings is not None else "No")

    # Main query area
    query = st.text_area("Enter your SIB research query:", height=100,
                         placeholder="e.g., Why can't sodium intercalate into graphite like lithium does?")

    if st.button("🔍 Analyze with LLM + GNN") and query:
        with st.spinner("Routing query through LLM and extracting GNN subgraph..."):
            analysis, focused_G = pipeline.run_query(query, top_k_nodes=15)

        # Display analysis results
        display_analysis_results(analysis)

        # Visualize
        tab1, tab2, tab3 = st.tabs(["📊 Literature Graph", "📋 Problem Summary", "🔗 Causal Chain"])
        with tab1:
            if PYVIS_AVAILABLE:
                html = pipeline.visualizer.create_pyvis_graph(focused_G, analysis)
                st.components.v1.html(html, height=700)
            else:
                fig = pipeline.visualizer.create_matplotlib_graph(focused_G, analysis)
                st.pyplot(fig)
        with tab2:
            summary = pipeline.visualizer.create_problem_summary(analysis)
            display_problem_summary(summary)
        with tab3:
            fig = pipeline.visualizer.create_causal_chain_diagram(analysis)
            st.pyplot(fig)

    elif not query:
        st.info("👈 Load the database and train the GNN, then enter a query.")
